...
author: Lin fanjie
description:
- This module provides declarative management of VLANs on Maipu switches.
version_added: 1.0.0
notes:
...
from ansible_collections.maipu.maipu.plugins.module_utils.network.maipu.maipu import (
    load_config,
    run_commands,
    normalize_interface,
)
...
def parse_to_obj(logical_rows):
    ...
    obj = re.match(
        "(?P<no_id>\\d+)\\s+(?P<vlan_id>\\d+)\\s+(?P<name>[^\\s]+)\\s+(?P<state>[^\\s]+)\\s+(?P<mode>[^\\s]+)\\s*(?P<interfaces>.*)",
        first_row,
    ).groupdict()
...
def map_config_to_obj(module):
    return parse_vlan_brief(run_commands(module, ["show vlan"])[0])
