...
class TestFtdInstall(object):
    ...
    def has_kick_mock(self, mocker):
        return mocker.patch('ansible_collections.community.network.plugins.module_utils.network.ftd.device.HAS_KICK', True)

    def test_module_should_fail_when_kick_is_not_installed(self, mocker):
        mocker.patch('ansible_collections.community.network.plugins.module_utils.network.ftd.device.HAS_KICK', False)
